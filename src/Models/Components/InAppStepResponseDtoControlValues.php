<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace novu\Models\Components;


/** InAppStepResponseDtoControlValues - Control values for the in-app step */
class InAppStepResponseDtoControlValues
{
    /**
     * JSONLogic filter conditions for conditionally skipping the step execution. Supports complex logical operations with AND, OR, and comparison operators. See https://jsonlogic.com/ for full typing reference.
     *
     * @var ?array<string, mixed> $skip
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('skip')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string, mixed>|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?array $skip = null;

    /**
     * Content/body of the in-app message. Required if subject is empty.
     *
     * @var ?string $body
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('body')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $body = null;

    /**
     * Subject/title of the in-app message. Required if body is empty.
     *
     * @var ?string $subject
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('subject')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $subject = null;

    /**
     * URL for an avatar image. Must be a valid URL or start with / or {{"{{"}} variable }}.
     *
     * @var ?string $avatar
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('avatar')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?string $avatar = null;

    /**
     * Primary action button details.
     *
     * @var ?ActionDto $primaryAction
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('primaryAction')]
    #[\Speakeasy\Serializer\Annotation\Type('\novu\Models\Components\ActionDto|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?ActionDto $primaryAction = null;

    /**
     * Secondary action button details.
     *
     * @var ?ActionDto $secondaryAction
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('secondaryAction')]
    #[\Speakeasy\Serializer\Annotation\Type('\novu\Models\Components\ActionDto|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?ActionDto $secondaryAction = null;

    /**
     * Redirection URL configuration for the main content click (if no actions defined/clicked)..
     *
     * @var ?RedirectDto $redirect
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('redirect')]
    #[\Speakeasy\Serializer\Annotation\Type('\novu\Models\Components\RedirectDto|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?RedirectDto $redirect = null;

    /**
     * Additional data payload for the step.
     *
     * @var ?array<string, mixed> $data
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('data')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string, mixed>|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?array $data = null;

    /**
     * $additionalProperties
     *
     * @var ?array<string, mixed> $additionalProperties
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('additionalProperties')]
    #[\Speakeasy\Serializer\Annotation\Type('array<string, mixed>|null')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?array $additionalProperties = null;

    /**
     * Disable sanitization of the output.
     *
     * @var ?bool $disableOutputSanitization
     */
    #[\Speakeasy\Serializer\Annotation\SerializedName('disableOutputSanitization')]
    #[\Speakeasy\Serializer\Annotation\SkipWhenNull]
    public ?bool $disableOutputSanitization = null;

    /**
     * @param  ?array<string, mixed>  $skip
     * @param  ?string  $body
     * @param  ?string  $subject
     * @param  ?string  $avatar
     * @param  ?ActionDto  $primaryAction
     * @param  ?ActionDto  $secondaryAction
     * @param  ?RedirectDto  $redirect
     * @param  ?bool  $disableOutputSanitization
     * @param  ?array<string, mixed>  $data
     * @param  ?array<string, mixed>  $additionalProperties
     * @phpstan-pure
     */
    public function __construct(?array $skip = null, ?string $body = null, ?string $subject = null, ?string $avatar = null, ?ActionDto $primaryAction = null, ?ActionDto $secondaryAction = null, ?RedirectDto $redirect = null, ?array $data = null, ?array $additionalProperties = null, ?bool $disableOutputSanitization = false)
    {
        $this->skip = $skip;
        $this->body = $body;
        $this->subject = $subject;
        $this->avatar = $avatar;
        $this->primaryAction = $primaryAction;
        $this->secondaryAction = $secondaryAction;
        $this->redirect = $redirect;
        $this->data = $data;
        $this->additionalProperties = $additionalProperties;
        $this->disableOutputSanitization = $disableOutputSanitization;
    }
}