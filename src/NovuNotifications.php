...
class NovuNotifications
{
     ...
     */
    public function feed(Operations\SubscribersV1ControllerGetNotificationsFeedRequest $request, ?Options $options = null): Operations\SubscribersV1ControllerGetNotificationsFeedResponse
    {
        $retryConfig = null;
        if ($options) {
        ...
        $hookContext = new HookContext('SubscribersV1Controller_getNotificationsFeed', null, $this->sdkConfiguration->securitySource);
     ...
     * @throws \novu\Models\Errors\APIException
     */
    public function unseenCount(string $subscriberId, ?bool $seen = null, ?float $limit = null, ?string $idempotencyKey = null, ?Options $options = null): Operations\SubscribersV1ControllerGetUnseenCountResponse
    {
        $retryConfig = null;
        if ($options) {
            $retryConfig = $options->retryConfig;
        ...
        $request = new Operations\SubscribersV1ControllerGetUnseenCountRequest(
        ...
        $qp = Utils\Utils::getQueryParams(Operations\SubscribersV1ControllerGetUnseenCountRequest::class, $request, $urlOverride);
        $httpOptions = array_merge_recursive($httpOptions, Utils\Utils::getHeaders($request));
        if (! array_key_exists('headers', $httpOptions)) {
            $httpOptions['headers'] = [];
        }
        ...
        $httpOptions['headers']['user-agent'] = $this->sdkConfiguration->userAgent;
        $httpRequest = new \GuzzleHttp\Psr7\Request('GET', $url);
        $hookContext = new HookContext('SubscribersV1Controller_getUnseenCount', null, $this->sdkConfiguration->securitySource);
        $httpRequest = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookContext), $httpRequest);
        $httpOptions['query'] = Utils\QueryParameters::standardizeQueryParams($httpRequest, $qp);
        $httpOptions = Utils\Utils::convertHeadersToOptions($httpRequest, $httpOptions);
        $httpRequest = Utils\Utils::removeHeaders($httpRequest);
        ...
        if (Utils\Utils::matchStatusCodes($statusCode, ['200'])) {
            if (Utils\Utils::matchContentType($contentType, 'application/json')) {
                ...
                $response = new Operations\SubscribersV1ControllerGetUnseenCountResponse(
                    statusCode: $statusCode,
                    contentType: $contentType,
                    rawResponse: $httpResponse,
                    headers: $httpResponse->getHeaders(),
